...
class Data1Pipeline:
    ...
    def cast_types(self, df):
        return df.astype({
            "code": "Int64", "name": "string", "gemeinde": "category",
            "adresse": "string", "strasse": "string", "richtung": "string",
            "jahr": "Int64", "wochentag": "category"
        })
    ...
    def rename_columns(self, df):
        return df.rename(columns={
            "reg_bus": "Regional Bus", "mr": "Motorrad", "pw": "Personenwagen",
            "pw+": "Personenwagen mit anhänger", "lief": "Lieferwagen",
            "lief+": "Lieferwagen mit anhänger", "lief+aufl.": "Lieferwagen mit auflieger",
            "lw": "Lastwagen", "lw+": "Lastwagen mit anhänger"
        })
    ...
    def aggregate_vehicle_counts(self, df):
        ...
        df["lieferwagen_total"] = df["Lieferwagen"] + df["Lieferwagen mit anhänger"] + df["Lieferwagen mit auflieger"]
        df["lkw_total"] = df["Lastwagen"] + df["Lastwagen mit anhänger"] + df["sattelzug"]
        df["motorisiert_total"] = df["Motorrad"] + df["bus"] + df["pkw_total"] + df["lieferwagen_total"] + df[
            "lkw_total"]
        return df
...
class Data2Pipeline:
    ...
    def cast_types(self, df):
        return df.astype({"messstelle": "category", "messstationid": "string", "indikator": "category"})
    ...
    def run(self):
        ...
        mask = df["messstelle"].astype(str).str.contains(self.cfg.gemeinde_filter, case=False, na=False)
        return df[mask].copy()
...
class MergePipeline:
    ...
    def run(self, df1, df2):
        for df in (df1, df2):
            ...
            df["zeit_von"] = pd.to_datetime(df["zeit_von"].astype(str)).dt.strftime("%H:%M")

        merged = df1.merge(df2, on=self.cfg.join_keys, how=self.cfg.join_type, suffixes=("_d1", "_d2"))
        merged.drop(columns=self.cfg.drop_columns, inplace=True, errors="ignore")

        streets = {s: merged[merged["strasse"] == s].copy() for s in self.cfg.strassen_filter}
